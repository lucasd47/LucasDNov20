public class WhileLoopFun {
    ...
    public int countLetter(String word, String letter) {
        ...
        while (currentChar < word.length()){
            String currentString = word.substring(currentChar, currentChar + 1);
            if (currentString.equals(letter)){
                totalCount++;
            }
            currentChar++;
        }
        if (currentChar == word.length()){
            String currentString = word.substring(currentChar);
            if (currentString.equals(letter)){
                totalCount++;
            }
            currentChar++;
    ...
    public int maxDoubles(int number, int threshold) {
        int countMultiplication2 = 0;
        while (number < threshold){
            if (number * 2 > threshold){
                break;
            }
            countMultiplication2++;
            number*=2;
        }
        return countMultiplication2;
    }
